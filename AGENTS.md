...
3. `JwtFilter`가 토큰 검증 후 `SecurityContext`에 유저 정보 저장
...
- 전역 예외는 `GlobalExceptionHandler`에서 처리
...
1. **회원가입 / 로그인** — 이메일 + 비밀번호, 이메일 중복 불가
2. **장기 목표 관리** — 제목, 설명, 마감일 설정 / 수정 / 삭제
3. **하루 계획 관리** — 날짜별 생성, 장기 목표와 연결 가능
4. **Task 관리** — 세부 할 일 등록 / 완료 체크 / 수정 / 삭제
5. **진행률 조회** — 장기 목표별 Task 달성률을 퍼센트로 시각화

---

## 스프린트 계획

### Sprint 1 (~5/11) — 문서 작성 및 인증
- **Dev**: 초기 세팅, 회원가입/로그인(JWT), User 엔티티, Spring Security 필터
- **Test**: AuthService 단위 테스트, POST /auth/signup 통합 테스트, POST /auth/login + JWT 검증

### Sprint 2 (~5/13) — 핵심 CRUD API
- **Dev**: 장기 목표 CRUD API, 하루 계획 CRUD API, Task 등록·체크·삭제 API
- **Test**: GoalService 단위 테스트, CRUD API MockMvc 통합 테스트, Task 완료 체크 동작 검증

### Sprint 3 (~5/18) — 진행률 & 배포
- **Dev**: 진행률 계산 API (달성률 %), 프로그레스 바 시각화 UI, Railway 배포 & 환경 설정
- **Test**: 진행률 계산 로직 단위 테스트, 전체 플로우 E2E 시나리오 테스트, 배포 후 API 연동 검증

### Sprint 4 (~5/20) — UI 통합 & 고도화
- **Dev**: React UI 통합 & UX 개선, API 에러 핸들링 고도화, Swagger API 문서 자동화
- **Test**: UI 컴포넌트 단위 테스트, 에러 응답 코드 검증

### Sprint 5 (~5/25) — 리팩터링 & 커버리지
- **Dev**: AI-assisted 코드 리팩터링, JaCoCo 커버리지 측정, PR 기반 코드 리뷰
- **Test**: 커버리지 80% 이상 달성, 리팩터링 후 회귀 테스트

### Sprint 6 (~5/27) — 최종 마무리
- **Dev**: 최종 통합 테스트 & 버그 수정, Railway 최종 배포 검증, README 작성
- **Test**: 전체 E2E 최종 시나리오 검증, 배포 URL 접속 최종 확인